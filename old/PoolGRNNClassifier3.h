...
#ifndef SRC_PoolGRNNClassifier3_H_
#define SRC_PoolGRNNClassifier3_H_
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, Options options) {
    ...
    _positionEmbSize = options.otherEmbSize;
    ...
    _rnn_left.initial(_rnnHiddenSize, _inputsize + 2*_positionEmbSize, true, 10);
    _rnn_right.initial(_rnnHiddenSize, _inputsize + 2*_positionEmbSize, false, 40);
    ...
    cout<<"PoolGRNNClassifier initial"<<endl;
    cout<<"add position emb"<<endl;
  }
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      Tensor<xpu, 3, dtype> position1prime, position1primeLoss;
      Tensor<xpu, 3, dtype> position2prime, position2primeLoss;
      Tensor<xpu, 3, dtype> position_input, position_inputLoss;
      ...
      position1prime = NewTensor<xpu>(Shape3(seq_size, 1, _positionEmbSize), 0.0);
      position1primeLoss = NewTensor<xpu>(Shape3(seq_size, 1, _positionEmbSize), 0.0);
      position2prime = NewTensor<xpu>(Shape3(seq_size, 1, _positionEmbSize), 0.0);
      position2primeLoss = NewTensor<xpu>(Shape3(seq_size, 1, _positionEmbSize), 0.0);
      position_input = NewTensor<xpu>(Shape3(seq_size, 1, _inputsize+2*_positionEmbSize), 0.0);
      position_inputLoss = NewTensor<xpu>(Shape3(seq_size, 1, _inputsize+2*_positionEmbSize), 0.0);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
       ...
       _words.GetEmb(words[0], wordprime[idx]);
        ...
        _position.GetEmb(feature.position1, position1prime[idx]);
        _position.GetEmb(feature.position2, position2prime[idx]);
      }
      ...
      for (int idx = 0; idx < seq_size; idx++) {
    	  concat(input[idx], position1prime[idx], position2prime[idx], position_input[idx]);
      }


      _rnn_left.ComputeForwardScore(position_input, rnn_hidden_left_reset, rnn_hidden_left_afterreset, rnn_hidden_left_update, rnn_hidden_left_current, rnn_hidden_left);
      _rnn_right.ComputeForwardScore(position_input, rnn_hidden_right_reset, rnn_hidden_right_afterreset, rnn_hidden_right_update, rnn_hidden_right_current, rnn_hidden_right);
/*
...
*/
      ...
      _rnn_left.ComputeBackwardLoss(position_input, rnn_hidden_left_reset, rnn_hidden_left_afterreset, rnn_hidden_left_update, rnn_hidden_left_current, rnn_hidden_left, rnn_hidden_leftLoss, position_inputLoss);
      _rnn_right.ComputeBackwardLoss(position_input, rnn_hidden_right_reset, rnn_hidden_right_afterreset, rnn_hidden_right_update, rnn_hidden_right_current, rnn_hidden_right, rnn_hidden_rightLoss, position_inputLoss);

      for (int idx = 0; idx < seq_size; idx++) {
    	  unconcat(inputLoss[idx], position1primeLoss[idx], position2primeLoss[idx], position_inputLoss[idx]);
      }
...
*/
      ...
      for (int idx = 0; idx < seq_size; idx++) {
    	  const Feature& feature = example.m_features[idx];
          _position.EmbLoss(feature.position1, position1primeLoss[idx]);
          _position.EmbLoss(feature.position2, position2primeLoss[idx]);
      }
      ...
      FreeSpace(&position1prime);
      FreeSpace(&position1primeLoss);
      FreeSpace(&position2prime);
      FreeSpace(&position2primeLoss);
      FreeSpace(&position_input);
      FreeSpace(&position_inputLoss);
  ...
  int predict(const Example& example, vector<dtype>& results) {
    ...
    Tensor<xpu, 3, dtype> position1prime;
    Tensor<xpu, 3, dtype> position2prime;
    Tensor<xpu, 3, dtype> position_input;
...
concat(beforerepresent, formerrepresent, middlerepresent, latterrepresent, afterrepresent, poolmerge);
  ...
  void updateParams(dtype nnRegular, dtype adaAlpha, dtype adaEps) {
    ...
    _position.updateAdaGrad(nnRegular, adaAlpha, adaEps);
  }
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    checkgrad(examples, _position._E, _position._gradE, "_position._E", iter, _position._indexers);
  }
