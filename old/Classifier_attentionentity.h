...
#ifndef CLASSIFIER_ATTENTIONENTITY_H_
#define CLASSIFIER_ATTENTIONENTITY_H_
...
class Classifier_attentionentity {
public:
	Options options;
	...
	Classifier_attentionentity(const Options& options):options(options) {
...
/*	virtual ~Classifier() {
	...
	void init(const NRMat<dtype>& wordEmb, const NRMat<dtype>& wordnetEmb, const NRMat<dtype>& brownEmb,
	    ...
	    output_layer.initial(_outputSize, options.hiddenSize, true, (int)time(0), 1);
	}
...
#endif /* CLASSIFIER_ATTENTIONENTITY_H_ */
