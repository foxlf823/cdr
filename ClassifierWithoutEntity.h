...
#ifndef CLASSIFIERWITHOUTENTITY_H_
#define CLASSIFIERWITHOUTENTITY_H_
...
public:
	...
	ClassifierWithoutEntity(const Options& options):options(options) {

	}
	virtual ~ClassifierWithoutEntity() {
	...
	void predict(const Example& example, vector<double>& scores) {
		...
		if(options.input_represent ==1) {
			...
			unit_att_before.ComputeForwardScore(input_before, y_before[beforeSize-1],
			...
			unit_att_middle.ComputeForwardScore(input_middle, y_middle[middleSize-1],
			...
			unit_att_after.ComputeForwardScore(input_after, y_after[afterSize-1],
	...
	dtype process(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			if(options.input_represent ==1) {
				...
				// hidden
				hidden_layer.ComputeBackwardLoss(y_att_before,
						y_att_middle, y_att_after,
						hidden, hiddenLoss,
						ly_att_before, ly_att_middle,
						ly_att_after);

				// attention

				unit_att_before.ComputeBackwardLoss(input_before, y_before[beforeSize-1],
						xMExp_before, xExp_before, xSum_before,
						xPoolIndex_before, y_att_before,
						ly_att_before, inputLoss_before, loss_before[beforeSize-1]);

				unit_att_middle.ComputeBackwardLoss(input_middle, y_middle[middleSize-1],
						xMExp_middle, xExp_middle, xSum_middle,
						xPoolIndex_middle, y_att_middle,
						ly_att_middle, inputLoss_middle, loss_middle[middleSize-1]);

				unit_att_after.ComputeBackwardLoss(input_after, y_after[afterSize-1],
						xMExp_after, xExp_after, xSum_after,
						xPoolIndex_after, y_att_after,
						ly_att_after, inputLoss_after, loss_after[afterSize-1]);
